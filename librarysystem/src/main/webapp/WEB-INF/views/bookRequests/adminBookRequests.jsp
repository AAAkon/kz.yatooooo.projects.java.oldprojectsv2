                ...
                <table class="table table-hover table-bordered dataTable" id="data-table">
                    ...
                    <c:if test="${bookRequests.size() > 0 }">
                        <c:forEach var="bookRequest" items="${bookRequests}">
                        	<c:choose>
                                <c:when test="${bookRequest.getStatus() == bookRequest.getStatusCodePending()}">
                            ...
                            <tr class='${bgClass}'>
                                ...
                                <td class="nowrap">
                                	<a href="seeBookRequest?id=${bookRequest.getId()}&redirectUrl=adminBookRequests" class='btn btn-warning btn-xs'>
	                                	<i class="fa fa-eye"> </i>
	                                	See
	                                </a>
	                                <c:if test="${bookRequest.getStatus() == bookRequest.getStatusCodePending()}">
	                                   	<a href="seeBookRequest?id=${bookRequest.getId()}&redirectUrl=adminBookRequests" class='btn btn-success btn-xs'>
		                                	<i class="fa fa-check"> </i>
		                                	Accept
		                                </a>
		                                <a href="seeBookRequest?id=${bookRequest.getId()}&redirectUrl=adminBookRequests" class='btn btn-danger btn-xs'>
		                                	<i class="fa fa-ban"> </i>
		                                	Decline
		                                </a>
	                                </c:if>
